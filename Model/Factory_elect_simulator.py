...
def variable_forecast_generator(conn, plant_mapping):
    ...
    variable_actual_Y23 = variable_actual_Y23.drop(columns={'site','last_update_time'}).rename(columns={'plant':'plant_code'})
    variable_actual_Y23 = pd.merge(variable_actual_Y23,
                                    plant_mapping, on=['plant_code'], how='left').reset_index(drop=True)
    variable_actual_Y23 = variable_actual_Y23.loc[~variable_actual_Y23.bo.isna(),:].reset_index(drop=True)
    ...
    variable_actual_Y23_summary = variable_actual_Y23.groupby(['bo','site','plant_name','account_name','date_key'], group_keys=True).agg({'value':'sum'}).reset_index().rename(columns={'value':'value_total','date_key':'datetime'})
    variable_actual_Y23_summary_wide = pd.pivot(variable_actual_Y23_summary, index=['bo','site','plant_name','datetime'], columns = 'account_name',values = 'value_total').rename_axis(None, axis=1).reset_index() 
    variable_actual_Y23_summary_wide['plant_name'] = np.where(variable_actual_Y23_summary_wide['plant_name']=='WCD-1','WCD',variable_actual_Y23_summary_wide['plant_name'])
...
def baseline_data_loader(conn, query, plant_mapping):
    ...
    data_result['plant'] = np.where(data_result['plant']=='WCD-1','WCD',data_result['plant'])
    
    # Merge actual variable and 
    variable_predict_Y23_summary_wide, variable_actual_Y23_summary_wide = variable_forecast_generator(conn, plant_mapping)
    ...
    variable_actual_Y23_summary_wide['datetime'] = variable_actual_Y23_summary_wide['datetime'].astype(str)
    ...
    data_result = data_result.loc[(~data_result['average_temperature'].isna()) & (~data_result['factory_electricity'].isna()),:].reset_index(drop=True)
    
    # close the communication with the PostgreSQL
    # cur.close()

    data_result_history = data_result[data_result.columns[np.where([x.find('id')==-1 for x in data_result.columns])]]
    data_result_history.columns = ['bo','site','plant','日期','人數（人）','產量（pcs)','出貨量（pcs)','工廠用電（kwh）','空調用電（kwh）','空壓用電（kwh）','生產用電（kwh）','基礎用電（kwh）','宿舍用電（kwh）',
                                   '外氣平均溫度（℃）','last_update_time']
...
def history_data_fixer(data_result, data_result_history):
    ...
    data_result = pd.merge(data_result,
                           data_result.loc[(~data_result.ac_electricity.isna()) & (~data_result.shipment_qty.isna()) & (~data_result.product_qty.isna()),:].groupby(['plant','bo']).agg({'datetime':'max'}).reset_index().rename(columns={'datetime':'datetime_max'}),
                           on=['plant','bo'], how='left')
    return data_result, data_result_history
...
def factory_elct_main_fn():
    ...
    try:
        ...
        baseline_year = '2022'
        target_year = '2030'
        plant_batch = [['WOK'],['WZS-1'],['WZS-3'],['WZS-6'],['WZS-8'],['WTZ'],['WCD'],['WKS-5'],['WKS-6'],['WCQ'],['WIH'],['KOE'],['WGKS'],['XTRKS'],['WIHK-2']]
        base_end_month = (datetime.now() - pd.DateOffset(months=1)).strftime('%Y-%m-%d')
        base_start_month = str(np.datetime64(base_end_month[0:7]) - np.timedelta64(11, 'M'))+'-01'
        ...
        data_result, data_result_history = baseline_data_loader(conn, load_baseline_data_query, plant_mapping)
        ...
        ##----- consider Y22-07~Y23-06-----##
        # Y22 by month factory real electricity
        predict_site = ['WGTX','WVN','WTN'] # no Y22 data
        ...
        model_site = ['XTRKS','KOE','WGKS','WZS','WKS','WCD','WCQ','WIH','WOK','WTZ']
        ...
        data_upload_final_Y23base_final_bymonth = data_upload_final_Y23base_final_bymonth.drop_duplicates().reset_index(drop=True)
        ...
        data_uploader_append(decarb_elect_simulate.loc[decarb_elect_simulate.version!='base',:],'app','decarb_elect_simulate')
