...
def variable_data_generator(data_result):
    ...
    data_result_trend_shipment_qty = data_result_trend.loc[~data_result_trend.shipment_qty.isna(),:].groupby(['plant','bo','year']).agg({'shipment_qty':'sum','datetime':'size'}).reset_index().rename(columns={'shipment_qty':'year_shipment_qty','datetime':'month_cnt'})
    data_result_trend_product_qty = data_result_trend.loc[~data_result_trend.product_qty.isna(),:].groupby(['plant','bo','year']).agg({'product_qty':'sum','datetime':'size'}).reset_index().rename(columns={'product_qty':'year_product_qty','datetime':'month_cnt'})
    ...
    # Compute trend rate
    data_result_trend_shipment_qty_min = trend_rate_generator(data_result_trend_shipment_qty,'shipment_qty','irr')
    data_result_trend_product_qty_min = trend_rate_generator(data_result_trend_product_qty,'product_qty','irr')
    # data_result_trend_fa_qty_min = trend_rate_generator(data_result_trend_fa_qty,'fa_qty','mape')
    data_result_trend_member_counts_min = trend_rate_generator(data_result_trend_member_counts,'member_counts','irr')
    ...
    data_result_trend_final = pd.merge(data_result_trend_shipment_qty_min,
                                         pd.merge(data_result_trend_member_counts_min,
                                                  ...
                                                  on=['plant','bo'], how='left'), 
                                         on=['plant','bo'], how='left')
    data_result_trend_final.loc[data_result_trend_final.product_qty_rate.isna(),['product_qty_rate']] = np.median(data_result_trend_final.loc[~data_result_trend_final.product_qty_rate.isna(),['product_qty_rate']])
...
def simulation_data_generator(conn, data_result, data_result_history, data_result_trend_final, baseline_year, target_year):
    ...
    data_result_simulation_all_plant_rate = data_result_simulation_all.loc[(data_result_simulation_all.plant.isin(['WKS-5','WKS-6','WZS-1','WZS-3','WZS-6','WZS-8','WIHK-1','WIHK-2'])) & (data_result_simulation_all.datetime.astype(str)>='2022-01-01'),:]
    data_result_simulation_all_plant_rate['Month'] = [int(str(x)[5:7]) for x in data_result_simulation_all_plant_rate['datetime']]
    data_result_simulation_all_plant_rate['Site'] = [str(np.where(real_id.find('-')!=-1,str(real_id[0:(real_id.find('-'))]),real_id)) for real_id in data_result_simulation_all_plant_rate.plant]
    ...
    plant_predict_energy_sub = "SELECT * FROM app.elect_target_month where (last_update_time='"+max_version+"') and (category='predict');"
    ...
    for x in range(1,(int(target_year)-int(baseline_year)+1)):
        ...
        # fix WMY member counts rate
        data_result_simulation.loc[(data_result_simulation.site=='WMY'),'member_counts_rate'] = abs(data_result_simulation.loc[(data_result_simulation.site=='WMY'),'member_counts_rate'])
        for factor in ['shipment_qty','product_qty','member_counts']: #,'ac_electricity','ap_electricity'
            if factor=='member_counts':
                data_result_simulation[factor] = data_result_simulation[factor].astype('float')*(1+data_result_simulation[factor+'_rate'].astype('float')/2*(x+1))
            else:
        ...
        for factor_ori in ['出貨量（pcs)','產量（pcs)','人數（人）']: #,'空調用電（kwh）','空壓用電（kwh）'
            ...
            print(factor)
            if factor=='member_counts':
                data_result_history_simulation[factor_ori] = data_result_history_simulation[factor_ori].astype('float')*(1+data_result_history_simulation[factor+'_rate'].astype('float')/2*(x+1))
            else:
...
def mfg_workspace_elec_generator(conn, workspace_site, target_year, base_year):
    ...
    data_result_site_query = "SELECT * FROM app.elect_target_month where category='predict' and last_update_time='"+str(max_datetime.max_datetime[0])+"';"
    ...
    data_result_site = data_result_site[['site','month','year','amount']].rename(columns={'site':'Site','amount':'amount_actual'})
    data_result_site['datetime'] = [datetime.date(datetime.strptime(str(data_result_site.year[x])+'-'+str(data_result_site.month[x])+'-01','%Y-%m-%d')) for x in range(len(data_result_site.year))]
    data_result_site['year'] = [str(x)[0:4] for x in data_result_site.datetime]
    data_result_site['month'] = [str(x)[5:7] for x in data_result_site.datetime]
    data_result_site_sub = data_result_site.loc[~data_result_site.Site.isin(workspace_site)]
    # workspace actual factory electricity
    data_result_workspace_query = "SELECT * FROM app.baseline_data_overview where (datetime>='2022-01-01') and (datetime<='2022-12-01') and plant in ('"+"','".join(workspace_site)+"');"
    data_result_workspace = data_loader(conn, data_result_workspace_query)
    data_result_workspace_sub = data_result_workspace.loc[~data_result_workspace.factory_electricity.isna()].reset_index(drop=True)
    data_result_workspace_sub['Site'] = [str(np.where(real_id.find('-')!=-1,str(real_id[0:(real_id.find('-'))]),real_id)) for real_id in data_result_workspace_sub.plant]
    data_result_workspace_sub['Site'] = np.where(data_result_workspace_sub.Site=='WIHK',data_result_workspace_sub.plant,data_result_workspace_sub.Site)
    data_result_workspace_sub = data_result_workspace_sub.groupby(['Site','datetime'], group_keys=True).agg({'factory_electricity':'sum'}).reset_index().rename(columns={'factory_electricity':'amount_actual'})
    data_result_workspace_sub['year'] = [str(x)[0:4] for x in data_result_workspace_sub.datetime]
    data_result_workspace_sub['month'] = [str(x)[5:7] for x in data_result_workspace_sub.datetime]
    data_result_workspace_sub_forecst = data_result_site.loc[data_result_site.Site.isin(workspace_site)]
    data_result_workspace = data_result_workspace_sub.append(data_result_workspace_sub_forecst.rename(columns={'factory_electricity_predict':'amount_actual'})).reset_index(drop=True)
    ...
    for year_index in range(1,int(target_year)-int(workspace_max_year)+1,1):
        data_result_workspace_prev = data_result_workspace.loc[(data_result_workspace.year<str(int(workspace_max_year)+year_index)) & (data_result_workspace.year>=str(int(workspace_max_year)+year_index-2))]
        data_result_workspace_prev = data_result_workspace_prev.loc[data_result_workspace_prev.amount_actual>0].groupby(['Site','month'], group_keys=True).agg({'amount_actual':'mean'}).reset_index()
        data_result_workspace_prev['year'] = str(int(workspace_max_year)+year_index)
        data_result_workspace_prev['datetime'] = [datetime.date(datetime.strptime(str(data_result_workspace_prev.year[x])+'-'+str(data_result_workspace_prev.month[x])+'-01','%Y-%m-%d')) for x in range(len(data_result_workspace_prev.year))]
        data_result_workspace = data_result_workspace.append(data_result_workspace_prev).reset_index(drop=True)
    data_result_site_sub_y23 = data_result_site_sub.append(data_result_workspace.loc[data_result_workspace.year>=str(base_year)]).reset_index(drop=True)
...
def variable_3y_forecast_generator(conn, plant_mapping, target_year):
    ...
    y23_plant_predict_shipment_month = shipment_predict_Y23.loc[:,['site','year','month','amount']].reset_index(drop=True).rename(columns={'site':'Site','amount':'shipment_qty'})
    y23_plant_predict_shipment_month['shipment_qty'] = y23_plant_predict_shipment_month.shipment_qty*1000
    y23_plant_predict_shipment_month.loc[y23_plant_predict_shipment_month.Site=='WIHK','Site']='WIHK1'
    y23_plant_predict_shipment_month['datetime'] = [str(datetime.strptime(str(y23_plant_predict_shipment_month['year'][x])+'-'+str(y23_plant_predict_shipment_month['month'][x])+'-01','%Y-%m-%d'))[0:10] for x in range(len(y23_plant_predict_shipment_month['year']))]
    y23_plant_predict_shipment_month['month'] = [str(x)[5:7] for x in y23_plant_predict_shipment_month['datetime']]
    ...
    variable_predict_Y23 = pd.merge(variable_predict_Y23,
                                    plant_mapping, on=['plant_code'], how='left').reset_index(drop=True)
    variable_predict_Y23 = variable_predict_Y23.loc[~variable_predict_Y23.bo.isna(),:].reset_index(drop=True)
    variable_predict_Y23['account_name'] = [str(np.where(x.find('Headcount')!=-1,'member_counts',
                                                   np.where(x.find('Production')!=-1,'product_qty','shipment_qty'))) for x in variable_predict_Y23['accountnm']]
    variable_predict_Y23_summary = variable_predict_Y23.groupby(['bo','site','plant_name','account_name','date_key'], group_keys=True).agg({'value':'sum'}).reset_index().rename(columns={'value':'value_total','date_key':'datetime'})
    variable_predict_Y23_summary = variable_predict_Y23_summary.loc[variable_predict_Y23_summary.datetime.astype(str)>=str(max(variable_predict_Y23_summary.datetime))[0:4]+'-01-01',:]
    variable_predict_Y23_summary_wide = pd.pivot(variable_predict_Y23_summary, index=['bo','site','plant_name','datetime'], columns = 'account_name',values = 'value_total').rename_axis(None, axis=1).reset_index() 
    variable_predict_Y23_summary_wide['plant_name'] = np.where(variable_predict_Y23_summary_wide['plant_name']=='WCD-1','WCD',variable_predict_Y23_summary_wide['plant_name'])
    # variable_predict_Y23_summary_wide['shipment_qty'] = variable_predict_Y23_summary_wide.shipment_qty_mb+variable_predict_Y23_summary_wide.shipment_qty_nb
    
    variable_predict_Y23_summary_wide['site'] = np.where(variable_predict_Y23_summary_wide.site=='WIHK',variable_predict_Y23_summary_wide.plant_name,variable_predict_Y23_summary_wide.site)
    variable_predict_Y23_summary_wide_month_rate = variable_predict_Y23_summary_wide.copy()
    variable_predict_Y23_summary_wide_month_rate['year'] = [str(x)[0:4] for x in variable_predict_Y23_summary_wide_month_rate['datetime']]
    variable_predict_Y23_summary_wide_month_rate['month'] = [str(x)[5:7] for x in variable_predict_Y23_summary_wide_month_rate['datetime']]
    #######################################################################
    variable_predict_Y23_summary_month_plant_rate = pd.merge(variable_predict_Y23_summary_wide_month_rate.loc[variable_predict_Y23_summary_wide_month_rate.site.isin(['WZS','WKS'])],
                                                                  variable_predict_Y23_summary_wide_month_rate.groupby(['site','year','month']).agg({'shipment_qty':'sum'}).reset_index().rename(columns={'shipment_qty':'shipment_qty_total'}),
                                                                  on=['site','year','month'], how='left')
    variable_predict_Y23_summary_month_plant_rate['plant_month_rate'] = variable_predict_Y23_summary_month_plant_rate.shipment_qty/variable_predict_Y23_summary_month_plant_rate.shipment_qty_total
    ...
    y23_plant_predict_shipment_bymonth = pd.merge(y23_plant_predict_shipment_month.rename(columns={'Site':'site'}),
                                                variable_predict_Y23_summary_month_plant_rate[['site','plant_name','month','plant_month_rate']], 
                                                on=['site','month'], how='left')
    y23_plant_predict_shipment_bymonth['plant_month_rate'] = np.where(y23_plant_predict_shipment_bymonth['plant_month_rate'].isna(),1,y23_plant_predict_shipment_bymonth['plant_month_rate'])
    y23_plant_predict_shipment_bymonth['shipment_qty'] = y23_plant_predict_shipment_bymonth.plant_month_rate*y23_plant_predict_shipment_bymonth.shipment_qty
    y23_plant_predict_shipment_bymonth['plant_name'] = np.where(y23_plant_predict_shipment_bymonth['plant_name'].isna(),y23_plant_predict_shipment_bymonth['site'],y23_plant_predict_shipment_bymonth['plant_name'])
    ...
    y23_plant_predict_shipment_bymonth['datetime'] = [datetime.date(datetime.strptime(str(x),'%Y-%m-%d')) for x in y23_plant_predict_shipment_bymonth.datetime]
    #######################################################################
    variable_predict_Y23_summary_wide_month_rate = pd.merge(variable_predict_Y23_summary_wide_month_rate,
                                                 variable_predict_Y23_summary_wide_month_rate.groupby(['site','year']).agg({'shipment_qty':'sum'}).reset_index().rename(columns={'shipment_qty':'shipment_qty_total'}),
                                                on=['site','year'], how='left')
    variable_predict_Y23_summary_wide_month_rate['plant_month_rate'] = variable_predict_Y23_summary_wide_month_rate.shipment_qty/variable_predict_Y23_summary_wide_month_rate.shipment_qty_total
    # agg shipment data
    sql = 'SELECT max(last_update_time) as last_update_time FROM app."decarb_est_shipments";'
    shipment_predict_Y23 = pd.read_sql(sql, con=conn)
    max_datetime = str(shipment_predict_Y23['last_update_time'][0])
    ...
    y23_plant_predict_shipment_month = shipment_predict_Y23.loc[:,['site','year','month','amount']].reset_index(drop=True).rename(columns={'site':'Site','amount':'shipment_qty'})
    y23_plant_predict_shipment_month['shipment_qty'] = y23_plant_predict_shipment_month.shipment_qty*1000
    y23_plant_predict_shipment_sub = y23_plant_predict_shipment_month.groupby(['Site','year'], group_keys=True).agg({'shipment_qty':'sum'}).reset_index().rename(columns={'shipment_qty':'shipment_qty_total'})
    y23_plant_predict_shipment_sub.loc[y23_plant_predict_shipment_sub.Site=='WIHK','Site']='WIHK1'
    # modify to 09_ver.
    # y23_plant_predict_shipment_new = pd.read_excel('D:/ESG/脫碳減碳方案/用電量資料/Y23-Y25預估出貨量__20230915.xlsx',sheet_name='wide_0915')
    # y23_plant_predict_shipment_new.loc[y23_plant_predict_shipment_new.Site=='WIHK','Site']='WIHK1'
    # y23_plant_predict_shipment_sub_new = y23_plant_predict_shipment_new.groupby(['Site','year'], group_keys=True).agg({'amount':'sum'}).reset_index().rename(columns={'amount':'shipment_qty_total'})
    # y23_plant_predict_shipment_sub = y23_plant_predict_shipment_sub.loc[y23_plant_predict_shipment_sub.year>=2024].append(y23_plant_predict_shipment_sub_new.loc[y23_plant_predict_shipment_sub_new.year==2023]).reset_index(drop=True)

    # GR grouth data
    plant_predict_shipment = pd.DataFrame({})
    for x in range(1,4,1):
        plant_predict_shipment_sub = y23_plant_predict_shipment_sub.loc[y23_plant_predict_shipment_sub.year==min(y23_plant_predict_shipment_sub.year)+(x-1)].rename(columns={'year':'base_year','shipment_qty_total':'base_shipment_qty_total'})
        plant_predict_shipment_gr = pd.merge(plant_predict_shipment_sub,
                                          y23_plant_predict_shipment_sub.loc[y23_plant_predict_shipment_sub.year==min(y23_plant_predict_shipment_sub.year)+x],
                                         on=['Site'],how='left').reset_index(drop=True)
        plant_predict_shipment_gr['GR'] = (plant_predict_shipment_gr.shipment_qty_total-plant_predict_shipment_gr.base_shipment_qty_total)/plant_predict_shipment_gr.base_shipment_qty_total
        plant_predict_shipment = plant_predict_shipment.append(plant_predict_shipment_gr[['Site','year','GR']]).reset_index(drop=True)
    ...
    plant_predict_shipment_na = y23_plant_predict_shipment_sub.loc[(y23_plant_predict_shipment_sub.year==max(y23_plant_predict_shipment_sub.year)) & (~y23_plant_predict_shipment_sub.Site.isin(plant_predict_shipment.Site))]
    plant_predict_shipment_na = pd.merge(plant_predict_shipment_na,
                                         y23_plant_predict_shipment_sub.loc[(y23_plant_predict_shipment_sub.year==min(y23_plant_predict_shipment_sub.year)) & (y23_plant_predict_shipment_sub.Site.isin(plant_predict_shipment_na.Site))].rename(columns={'year':'base_year','shipment_qty_total':'base_shipment_qty_total'}),
                                         on=['Site'],how='left').reset_index(drop=True)
    plant_predict_shipment_na['GR']=(plant_predict_shipment_na.shipment_qty_total-plant_predict_shipment_na.base_shipment_qty_total)/plant_predict_shipment_na.base_shipment_qty_total/2
    ...
    plant_predict_shipment_na = plant_predict_shipment_na.append(plant_predict_shipment_na_previous).reset_index(drop=True)
    plant_predict_shipment = plant_predict_shipment.append(plant_predict_shipment_na[plant_predict_shipment.columns]).reset_index(drop=True)
    ...
    y23_plant_predict_shipment_sub = pd.merge(y23_plant_predict_shipment_sub.rename(columns={'Site':'site'}),
                                          variable_predict_Y23_summary_wide_month_rate[['site','plant_name','month','plant_month_rate']],
                                          on = ['site'], how = 'left').reset_index(drop=True)
    y23_plant_predict_shipment_sub['shipment_qty'] = y23_plant_predict_shipment_sub.shipment_qty_total*y23_plant_predict_shipment_sub.plant_month_rate
    y23_plant_predict_shipment_sub['datetime'] = [datetime.date(datetime.strptime(str(y23_plant_predict_shipment_sub.year[x])+'-'+str(y23_plant_predict_shipment_sub.month[x])+'-01','%Y-%m-%d')) for x in range(y23_plant_predict_shipment_sub.shape[0])]
    y23_plant_predict_shipment_sub = pd.merge(y23_plant_predict_shipment_sub,
                                              y23_plant_predict_shipment_bymonth[['plant_name','datetime','shipment_qty']].rename(columns={'shipment_qty':'shipment_qty_new'}),
    ...
    y23_plant_predict_shipment_sub = y23_plant_predict_shipment_sub.drop(columns={'shipment_qty'}).rename(columns={'shipment_qty_new':'shipment_qty'})
    ...
    variable_predict_Y23_summary_wide = variable_predict_Y23_summary_wide.drop(columns={'shipment_qty'}).rename(columns={'shipment_qty_new':'shipment_qty'})
...
def real_base_generator(conn, data_result_simulation_all, plant_predict_energy_all, base_start_month, base_end_month, predict_site):
    ...
    data_upload_final_Y25pred = data_result_simulation_all.loc[(data_result_simulation_all.datetime>=base_start_month) & (data_result_simulation_all.datetime<=base_end_month),['plant','datetime','factory_electricity']].rename(columns={'factory_electricity':'factory_electricity_base'}).reset_index(drop=True)
    data_upload_final_Y25pred['month'] = [x[5:7] for x in data_upload_final_Y25pred['datetime']]
...
def site_electricity_generator(data_upload_final, data_upload_final_Y23real, data_upload_final_Y25pred, base_year, base_start_month, base_end_month, model_site, plant_predict_shipment):
    ...
    data_upload_final_Y23base['Site'] = [str(np.where(real_id.find('-')!=-1,str(real_id[0:(real_id.find('-'))]),real_id)) for real_id in data_upload_final_Y23base.plant]
    data_upload_final_Y23base['Site'] = np.where(data_upload_final_Y23base.Site=='WIHK',data_upload_final_Y23base.plant,data_upload_final_Y23base.Site)  
    plant_predict_shipment_rate = plant_predict_shipment.loc[plant_predict_shipment.year.astype(int)==base_year+1]
    ...
    data_upload_final_Y23base = pd.merge(data_upload_final_Y23base, plant_predict_shipment_rate[['Site','GR']], 
                                         on=['Site'], how='left').reset_index(drop=True)
    data_upload_final_Y23base['month_grouth_rate'] = np.where((~data_upload_final_Y23base['Site'].isin(['WCD','WGKS'])) & (data_upload_final_Y23base['month_grouth_rate']<0) & (data_upload_final_Y23base['year'].astype(int)>=base_year+1),(data_upload_final_Y23base['year'].astype(int)-base_year)*data_upload_final_Y23base['GR'],data_upload_final_Y23base['month_grouth_rate']) # fix me !!! 0.01
    data_upload_final_Y23base = data_upload_final_Y23base.drop(columns={'Site'})
    # Adjust GR parameters-------
    data_upload_final_Y23base['month_grouth_rate'] = np.where((data_upload_final_Y23base['year'].astype(int)==base_year+1) & (data_upload_final_Y23base['plant'].isin(['KOE','WCQ','WIHK-2'])),(data_upload_final_Y23base['year'].astype(int)-base_year)*(-0.20),data_upload_final_Y23base['month_grouth_rate']) # fix me !!! 0.01
    data_upload_final_Y23base['month_grouth_rate'] = np.where((data_upload_final_Y23base['year'].astype(int)>=base_year+1) & (data_upload_final_Y23base['plant'].isin(['WMY-1'])),(data_upload_final_Y23base['year'].astype(int)-base_year)*(2.0),data_upload_final_Y23base['month_grouth_rate']) # fix me !!! 0.01
    data_upload_final_Y23base['month_grouth_rate'] = np.where((data_upload_final_Y23base['month_grouth_rate']<0) & (data_upload_final_Y23base['year'].astype(int)>=base_year+1) & (data_upload_final_Y23base['plant'].isin(['WOK','WIHK','WTZ'])),data_upload_final_Y23base['GR'],data_upload_final_Y23base['month_grouth_rate']) # fix me !!! 0.01
    data_upload_final_Y23base['month_grouth_rate'] = np.where((data_upload_final_Y23base['year'].astype(int)>base_year+1) & ~(data_upload_final_Y23base['plant'].isin(['WOK','WIHK','WTZ'])),(data_upload_final_Y23base['year'].astype(int)-base_year)*0.06,data_upload_final_Y23base['month_grouth_rate']) # fix me !!! 0.01
    data_upload_final_Y23base['month_grouth_rate'] = np.where((data_upload_final_Y23base['month_grouth_rate']/(data_upload_final_Y23base['year'].astype(int)-base_year)>=0.3) ,(data_upload_final_Y23base['year'].astype(int)-base_year)*0.3,data_upload_final_Y23base['month_grouth_rate']) # fix me !!! 0.01
    # Adjust month_grouth_rate for WCD
    data_upload_final_Y23base_ratemean = data_upload_final_Y23base.groupby(['year','plant']).agg({'month_grouth_rate':'mean'}).reset_index().rename(columns={'month_grouth_rate':'month_grouth_rate_mean'})
    data_upload_final_Y23base = pd.merge(data_upload_final_Y23base, data_upload_final_Y23base_ratemean,on=['year','plant'], how='left')
    data_upload_final_Y23base['month_grouth_rate'] = np.where((data_upload_final_Y23base['year'].astype(int)>=base_year+1) & (data_upload_final_Y23base['plant'].isin(['WCD','WGKS','WMY-1','WMX','WIHK-2'])),(data_upload_final_Y23base['year'].astype(int)-base_year)*data_upload_final_Y23base['month_grouth_rate_mean'],data_upload_final_Y23base['month_grouth_rate']) # fix me
    ...
    data_upload_final_Y23base = pd.merge(data_upload_final_Y23base,
                                         ...
                                         data_upload_final_Y25pred[['plant','month','factory_electricity_base']],
    ...
    total_energy = total_energy.loc[total_energy.plant!='WIHK',:].reset_index(drop=True)
    total_energy['Site_top'] = [str(np.where(real_id.find('-')!=-1,str(real_id[0:(real_id.find('-'))]),real_id)) for real_id in total_energy.plant]
    total_energy['Site_top'] = np.where(total_energy.Site_top=='WIHK',total_energy.plant,total_energy.Site_top)
    total_energy['factory_electricity_base'] = (total_energy['factory_electricity_base'].astype(str)).astype(float)
    # total_energy.loc[total_energy.Site_top=='WVN',['factory_electricity_base']] = list(plant_predict_energy_sub.loc[(plant_predict_energy_sub.Site=='WVN') & (plant_predict_energy_sub.year==base_year),'factory_electricity']) #fix me
    # total_energy.loc[total_energy.Site_top=='WVN',['factory_electricity_base']] = list(plant_predict_energy_sub.loc[(plant_predict_energy_sub.Site=='WVN') & (plant_predict_energy_sub.year==base_year+1),'factory_electricity']) #fix me
    # WVN adjust
    wvn_actual_mean = np.mean(data_upload_final_Y23real.loc[(data_upload_final_Y23real.plant=='WVN')  & (data_upload_final_Y23real.datetime<=base_end_month),'factory_electricity_base'])
    total_energy.loc[total_energy.Site_top=='WVN',['factory_electricity_base']] = [wvn_actual_mean*(1+0.5) for x in range(0,len(total_energy.loc[total_energy.Site_top=='WVN',['factory_electricity_base']]))] #fix me
    # total_energy.loc[total_energy.plant.isin(['WIHK-1','WIHK-2']),['plant']] = 'WIHK'
    total_energy = total_energy.groupby(['plant','datetime','month','Site_top']).agg({'factory_electricity_base':'sum'}).reset_index()
    ...
    total_energy = total_energy.loc[((total_energy.datetime>=base_start_month) & (total_energy.datetime<=base_end_month) & (~total_energy.Site_top.isin(['WVN','WTN'])))|((total_energy.datetime>=str(base_year)+'-01-01') & (total_energy.datetime<=str(base_year)+'-12-01') & (total_energy.Site_top.isin(['WVN','WTN'])))].reset_index(drop=True)
    # total_energy.loc[total_energy.Site_top=='WIHK',['factory_electricity_base']] = list(plant_predict_energy_sub.loc[(plant_predict_energy_sub.Site.isin(['WIHK-1','WIHK-2'])) & (plant_predict_energy_sub.year==base_year),['month','factory_electricity']].groupby(['month']).agg({'factory_electricity':'sum'}).reset_index()['factory_electricity']) #fix me
    ...
    data_upload_final_Y23base_other.loc[(data_upload_final_Y23base_other['datetime'].astype(str)>=str(base_year+1)+'-01-01') & (data_upload_final_Y23base_other['plant'].isin(['WVN'])),'predict_electricity_v2'] = data_upload_final_Y23base_other.loc[(data_upload_final_Y23base_other['datetime'].astype(str)>=str(base_year+1)+'-01-01') & (data_upload_final_Y23base_other['plant'].isin(['WVN'])),'predict_electricity_v2']*(1+0.25)
    data_upload_final_Y23base_other.loc[(data_upload_final_Y23base_other['datetime'].astype(str)>=str(base_year+1)+'-01-01') & (data_upload_final_Y23base_other['plant'].isin(['WMY-1'])),'predict_electricity_v2'] = data_upload_final_Y23base_other.loc[(data_upload_final_Y23base_other['datetime'].astype(str)>=str(base_year+1)+'-01-01') & (data_upload_final_Y23base_other['plant'].isin(['WMY-1'])),'predict_electricity_v2']*(1+0.2)
...
def factory_elct_main_fn():
    ...
    y23_plant_predict_shipment_sub['datetime'] = y23_plant_predict_shipment_sub.datetime.astype(
        str)
    # Negative GR site
    negative_gr_site = plant_predict_shipment.loc[(
        plant_predict_shipment.year==max(plant_predict_shipment.year)) & (
        plant_predict_shipment.GR<0),'Site'].tolist()
    # actual production
    variable_predict_Y23_summary_wide_ori, variable_actual_Y23_summary_wide_ori = variable_forecast_generator(conn, plant_mapping)
    variable_actual_Y23_summary_wide_actual = variable_actual_Y23_summary_wide_ori.loc[(
        variable_actual_Y23_summary_wide_ori.datetime.astype(str)>baseline_year+'-12-01') & (
        variable_actual_Y23_summary_wide_ori.datetime.astype(str)<=real_date),['plant_name','datetime','product_qty','member_counts']].reset_index(
        drop=True).rename(columns={'plant_name':'plant','product_qty':'product_qty_actual','member_counts':'member_counts_actual'})
    variable_actual_Y23_summary_wide_actual['datetime'] = variable_actual_Y23_summary_wide_actual.datetime.astype(
        str)
    ...
    data_result_simulation_all = pd.merge(data_result_simulation_all,
                                          ...
                                          on=['plant', 'datetime'], how='left').reset_index(drop=True)
    data_result_simulation_all = pd.merge(data_result_simulation_all, 
                                      variable_actual_Y23_summary_wide_actual,
                                      on=['plant','datetime'], how='left').reset_index(drop=True)
    for x in ['shipment_qty', 'member_counts', 'product_qty']:
        if (x == 'product_qty') | (x == 'member_counts'):
            data_result_simulation_all[x] = np.where((
                data_result_simulation_all[x+'_actual'].isna()), ## fixme
                data_result_simulation_all[x],
                data_result_simulation_all[x+'_actual'])
        data_result_simulation_all[x] = np.where((
            data_result_simulation_all[x+'_forecast'].isna()) | (data_result_simulation_all['datetime']<=real_date), ##fixme
            data_result_simulation_all[x],
            data_result_simulation_all[x+'_forecast'])
    data_result_simulation_all['shipment_qty'] = np.where(
        data_result_simulation_all['shipment_qty_forecast_v2'].isna(), ##fixme
    ...
    data_result_history_simulation_all = pd.merge(data_result_history_simulation_all,
                                                          variable_predict_Y23_summary_wide[['plant_name','datetime','shipment_qty','member_counts','product_qty']].rename(columns={'plant_name':'plant','shipment_qty':'shipment_qty_forecast','member_counts':'member_counts_forecast','product_qty':'product_qty_forecast','datetime':'日期'}), # fix me
                                              on=['plant','日期'],how='left').reset_index(drop=True)
    data_result_history_simulation_all = pd.merge(data_result_history_simulation_all,
                                                  y23_plant_predict_shipment_sub.loc[y23_plant_predict_shipment_sub.datetime.astype(str)>='2024-01-01',['plant_name','datetime','shipment_qty']].rename(columns={'plant_name':'plant','shipment_qty':'shipment_qty_forecast_v2','datetime':'日期'}), # fix me
                                                  on=['plant','日期'],how='left').reset_index(drop=True)
    data_result_history_simulation_all = pd.merge(data_result_history_simulation_all, 
                                                  variable_actual_Y23_summary_wide_actual.rename(columns={'datetime':'日期'}),
                                                  on=['plant','日期'], how='left').reset_index(drop=True)
    for x in ['出貨量（pcs)','人數（人）','產量（pcs)']:
        x_name = str(np.where(x=='工廠用電（kwh）','factory_electricity',
                         np.where(x=='出貨量（pcs)','shipment_qty',
                                  np.where(x=='人數（人）','member_counts','product_qty'))))
        print(x_name)
        if (x_name == 'product_qty') | (x_name == 'member_counts'):
            data_result_history_simulation_all[x] = np.where((data_result_history_simulation_all[x_name+'_actual'].isna()), ## fixme
                                                                    data_result_history_simulation_all[x],
                                                                    data_result_history_simulation_all[x_name+'_actual'])
        data_result_history_simulation_all[x] = np.where((data_result_history_simulation_all[x_name+'_forecast'].isna())  | (data_result_history_simulation_all['日期']<=real_date),
                                                         data_result_history_simulation_all[x],
                                                         data_result_history_simulation_all[x_name+'_forecast'])
    data_result_history_simulation_all['出貨量（pcs)'] = np.where(data_result_history_simulation_all['shipment_qty_forecast_v2'].isna(), ##fixme
                                                                 data_result_history_simulation_all['出貨量（pcs)'],
                                                                 data_result_history_simulation_all['shipment_qty_forecast_v2'])
    # data_result_history_simulation_all.loc[(data_result_history_simulation_all.site=='WCD') & (data_result_history_simulation_all['日期']>=str(base_year+1)+'-01-01'),'人數（人）'] = data_result_history_simulation_all.loc[(data_result_history_simulation_all.site=='WCD') & (data_result_history_simulation_all['日期']>=str(base_year+1)+'-01-01'),'人數（人）']*0.7 #fix me

    # shipment and product align
    data_result_history_simulation_all.loc[data_result_history_simulation_all.日期<=real_date,
                                           '出貨量（pcs)'] = data_result_history_simulation_all.loc[data_result_history_simulation_all.日期<=real_date,'產量（pcs)']
    data_result_history_simulation_all.loc[data_result_history_simulation_all.日期>real_date,
                                           '產量（pcs)'] = data_result_history_simulation_all.loc[data_result_history_simulation_all.日期>real_date,'出貨量（pcs)']
    ...
    for x in plant_batch:
        print(x)
        data_result_json = json.loads(data_result_simulation_all.fillna('null').loc[(data_result_simulation_all['plant'].isin(x))  & (data_result_simulation_all['datetime']>='2022-04-01'), :].drop(columns={
        ...
        data_result_history_json = json.loads(data_result_history_simulation_all.fillna('null').loc[(data_result_history_simulation_all['plant'].isin(x)) & (data_result_history_simulation_all['日期']>='2022-04-01'), :].drop(
    ...
    data_upload_final_Y23base_final_bymonth_bysite.loc[(data_upload_final_Y23base_final_bymonth_bysite.Site.isin(
        ['WKS'])) & (data_upload_final_Y23base_final_bymonth_bysite.year>str(base_year+1)),'factory_electricity_predict']=0
    data_upload_final_Y23base_final_bymonth_bysite.loc[(data_upload_final_Y23base_final_bymonth_bysite.Site.isin(
        ['WMY'])) & (data_upload_final_Y23base_final_bymonth_bysite.year>=str(base_year+1)),'factory_electricity_predict']=data_upload_final_Y23base_final_bymonth_bysite.loc[(
        data_upload_final_Y23base_final_bymonth_bysite.Site.isin(['WMY'])) & (data_upload_final_Y23base_final_bymonth_bysite.year>=str(base_year+1)),'factory_electricity_predict']*(1+0.40)
    # adjust Y27~Y30 -- fix me
    end_year_predict = data_upload_final_Y23base_final_bymonth_bysite.loc[(
        data_upload_final_Y23base_final_bymonth_bysite.year==str(base_year+3))].reset_index(drop=True)
    end_year_predict_all = pd.DataFrame({})
    for x in range(1,int(target_year)-int(end_year_predict['year'][0])+1,1):
        end_year_predict_new = end_year_predict.copy()
        end_year_predict_new['year'] = [str(y+x) for y in end_year_predict_new['year'].astype(int)]
        end_year_predict_new_negative = end_year_predict_new.loc[end_year_predict_new.Site.isin(negative_gr_site)].reset_index(drop=True)
        end_year_predict_new_negative['factory_electricity_predict'] = [y*(1+0.05)**x for y in end_year_predict_new_negative['factory_electricity_predict']]
        end_year_predict_new_positive = end_year_predict_new.loc[~end_year_predict_new.Site.isin(negative_gr_site)].reset_index(drop=True)
        end_year_predict_new_positive['factory_electricity_predict'] = [y*(1+0.05)**x for y in end_year_predict_new_positive['factory_electricity_predict']]
        end_year_predict_new = end_year_predict_new_negative.append(end_year_predict_new_positive).reset_index(drop=True)
        end_year_predict_all = end_year_predict_all.append(end_year_predict_new).reset_index(drop=True)
    data_upload_final_Y23base_final_bymonth_bysite = data_upload_final_Y23base_final_bymonth_bysite.loc[data_upload_final_Y23base_final_bymonth_bysite.year<str(base_year+4)].append(end_year_predict_all).reset_index(drop=True)
    ...
    decarb_max_datetime_query = f"""SELECT max(last_update_time) as last_update_time FROM app.decarb_elect_simulate where "version"  != 'addition' """
    ...
    ## ----------12.5. add WIH addition site data to decarb_elect_simulate ------------##
    load_addition_site_data_query = "SELECT * FROM app.decarb_elect_simulate where version='addition';"
    addition_site_data = data_loader(conn, load_addition_site_data_query)
    addition_site_data = addition_site_data.drop(columns={'id','validate','sign_off_id'})
    addition_site_data['last_update_time'] = decarb_elect_simulate['last_update_time'][0]
    addition_site_data['version'] = max_version
    addition_site_data['version_year'] = max_version_year
    addition_site_data['year'] = addition_site_data.year.astype(str)
    decarb_elect_simulate = decarb_elect_simulate.append(addition_site_data).reset_index(drop=True)
    decarb_elect_simulate = decarb_elect_simulate.groupby(['site','year','version','last_update_time','version_year']).agg(
